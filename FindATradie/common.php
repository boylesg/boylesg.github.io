<?php
	session_start();
	$g_strEmailManager = "greg@find-a-tradie.com.au";
	...
	{
		$dbDatabase = null;
		global $g_strEmailManager;
		
		try
		{		
			$dbDatabase = new mysqli("127.0.0.1", "root", "qDHt7vvFvsOvUPG5", "millhouse_db");
		}
		catch(Exception $e)
		{
			$strMsg = $e->getMessage();
	...
	function DoQuery($dbConnection, $strQuery)
	{
		...
		catch(Exception $e) 
		{
			PrintJavascriptLine("alert(\"'" . $e->getMessage() . "' with query '" . $strQuery . "'\");", 2, true);
  			//echo "ERROR: '". $e->getMessage() . "'<br><br>With query '" . $strQuery . "'.<br><br>" . $g_strEmailManager;
		}		
		return $result;
	}
